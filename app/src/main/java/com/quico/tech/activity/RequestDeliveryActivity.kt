...
import com.quico.tech.model.ServiceBodyParameters
        ...
        when (delivery_type) {
            DOOR_TO_DOOR -> {
                if (selected_address_id == 0) {
                    ...
                    viewModel.requested_serive_order= viewModel.requested_serive_order?.copy(delivery_type=DOOR_TO_DOOR)
                    createServiceOrder(selected_address_id)
                }
            }
            DROP_CENTER -> {
                viewModel.requested_serive_order= viewModel.requested_serive_order?.copy(delivery_type= DROP_CENTER)
                createServiceOrder(0)
            }
...
private fun createServiceOrder(address_id: Int) {

    Common.setUpChoicesAlert(this,
        viewModel.getLangResources().getString(R.string.confirm_service),
        viewModel.getLangResources().getString(R.string.sure_create_service),
            ...
            override fun onConfirm() {
                if (address_id != 0)
                    viewModel.requested_serive_order =
                        viewModel.requested_serive_order?.copy(address_id = address_id)

                viewModel.requested_serive_order?.let { it ->
                    Common.setUpProgressDialog(this@RequestDeliveryActivity)

                    val params = ServiceBodyParameters(it)
                    viewModel.createServiceOrder(params,
                        object : SharedViewModel.ResponseStandard {
